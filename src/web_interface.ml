...
(* Global trace config for web mode *)
let web_trace_config : Lsc_eval.trace_config option ref = ref None

(* Simplified eval for trace mode that collects trace steps *)
let eval_program_with_trace (p : program) =
  clear_output ();

  (* Create trace config for web mode *)
  let trace_cfg = Lsc_eval.make_trace_config ~web_mode:true true in
  web_trace_config := Some trace_cfg;

  (* Set up trace marker in environment *)
...
(* Run Stellogen code with trace and return HTML trace output *)
let trace_from_string (code : string) : (string, string) Result.t =
  try
    (* Parse from string *)
    let raw_exprs = Sgen_parsing.parse_from_string code in
    ...
    | Ok program -> (
      (* Evaluate with trace *)
      match eval_program_with_trace program with
      | Ok () ->
        (* Get trace steps and format as HTML *)
        let trace_html =
          match !web_trace_config with
          | Some cfg ->
            let steps = Lsc_eval.get_trace_steps cfg in
            if List.is_empty steps then
              "<div class='no-trace'>No execution steps (program only has \
               definitions or non-exec operations)</div>"
            else Lsc_eval.format_trace_steps_html steps
          | None -> "<div class='error'>Trace not available</div>"
        in
        (* Also include regular output if any *)
        let regular_output = get_output () in
        let full_output =
          if String.is_empty regular_output then trace_html
          else regular_output ^ "\n\n" ^ trace_html
        in
        Ok full_output
      | Error err -> (
